const supportedDialects = ['mysql', 'sqlserver', 'access', 'sqlite']

module.exports.generateSchema = (json, { dialect }) => {
    if (!dialect) {
        throw 'Dialect must be provided'
    }
    if (!supportedDialects.includes(dialect)) {
        throw `Dialect not supported. Only ${supportedDialects.join(',')} are supported`
    }
